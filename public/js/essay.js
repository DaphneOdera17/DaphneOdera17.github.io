	...
	diffDate: function (d, more = false) {
		...
		if (more) {
			...
			if (monthCount >= 1) {
				...
				result =
					parseInt(dayCount) + " " + GLOBAL_CONFIG.dateSuffix.day;
			} else if (hourCount >= 1) {
				result =
					parseInt(hourCount) + " " + GLOBAL_CONFIG.dateSuffix.hour;
			} else if (minuteCount >= 1) {
				result =
					parseInt(minuteCount) + " " + GLOBAL_CONFIG.dateSuffix.min;
			} else {
	...
	changeTimeInEssay: function () {
		document.querySelector("#bber") &&
			document.querySelectorAll("#bber time").forEach(function (e) {
				...
				(t.innerText = june.diffDate(datetime, true)),
